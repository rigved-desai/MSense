...
class pefile:
                    ...
                    if hex(fRVA).startswith('0x8000'):
                        if retVal:
                            if PY3:
                                ...
                                dllDict[currDLL].append("Ordinal: %s" % ((hex(fRVA), 16) ^ ('0x80000000', 16)))
                        else:
                            if PY3:
                                print(" load by ordinal: %s" % (int(hex(fRVA), 16) ^ int('0x80000000', 16)))
                            else:
                                print(" load by ordinal: %s" % ((hex(fRVA), 16) ^ ('0x80000000', 16)))
                    else:
                        try:
    ...
    def readPEHeader(self, peHeader):
        ...
        if self.peDict['01_machine'] == '4c01':
            self.peDict['01_machine'] = "332"
        elif self.peDict['01_machine'] == '6486':
            self.peDict['01_machine'] = "512"
        else:
            self.peDict['01_machine'] = "0.0"
    ...
    def printInfo(self):
        ...
        print("%s" % (self.peoptDict['03_minorlnkv']))
        print("%s" % (self.peoptDict['04_codesize']))
        print("%s" % (self.peoptDict['05_initsize']))
        print("%s" % (self.peoptDict['06_uninitsize']))
        print("%s" % (self.peoptDict['07_entrypoint']))
        print("%s" % (self.peoptDict['08_baseofcode']))
        print("%s" % (self.peoptDict['09_baseofdata']))
        print("%f" % (self.peoptDict['10_imagebase']))
        print("%s" % (self.peoptDict['11_sectionalignment']))
        print("%s" % (self.peoptDict['12_filealignment']))
        print("%s" % (self.peoptDict['13_majorop']))
############################################################################
